...
function applyAccent(hex, store = false) {
    if (hex === 'none') { applyNoAccent(store); return; }
...
function gnNotify(msg, type = 'info', duration = 3500) {
    const toast = document.getElementById('gnToast');
    const icon = toast.querySelector('.toast-icon');
    document.getElementById('gnToastMsg').textContent = msg;
...
function updateColorSchemeUI() {
    ...
    if (ogBtn) ogBtn.classList.toggle('active', tempColorScheme === 'og');
}
...
function openZone(file) {
    const chatFrame = document.getElementById('gnChat');
    ...
    fetch(url + '?t=' + Date.now())
        ...
        }).catch(e => gnNotify('Failed to load: ' + e, 'error'));
}
...
async function loadData(e) {
    ...
    reader.onload = ev => {
        try {
            ...
            if (!b.version || !b.localStorage) { gnNotify('Invalid backup file.', 'error'); return; }
            ...
            gnNotify('Restored! Close and reopen this tab to apply.', 'success', 5000);
        } catch(err) { gnNotify('Import failed: ' + err.message, 'error'); }
    };
